...
func post_import(tileset: TileSet) -> TileSet:
	...
	var tileset_data_layer_id := tileset.get_custom_data_layer_by_name("LDtk")
	...
	for y in range(0, grid_size.y):
		for x in range(0, grid_size.x):
			...
			if has_tile:
				...
				var tile_custom_data :Dictionary= tile_data.get_custom_data("LDtk")
				
				if tile_custom_data != null and tile_custom_data != {}:
					var tag :String= tile_custom_data.get("enum")
					if tag == "Collision":
